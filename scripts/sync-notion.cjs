...
const RESTAURANTS_DIR = path.join(process.cwd(), 'content/restaurants');
...
function findExistingFileByPageId(pageId) {
  const files = fs.readdirSync(RESTAURANTS_DIR).filter(file => file.endsWith('.md'));
...
function deleteRestaurantFile(slug) {
...
async function scheduledSync() {
  ...
  console.log(`📚 Found ${response.results.length} published restaurants (date < now)`);
  ...
  if (newPublishedSlugs.length > 0) {
    ...
    console.log(`\nℹ️  No new restaurants published`);
  }
...
async function webhookSync() {
  ...
  if (status === 'Deleted') {
    ...
    const deleted = deleteRestaurantFile(slug);
    return deleted;
  }
